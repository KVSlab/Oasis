...
def setup(u, q_, q_1, uc_comp, u_components, dt, v, U_AB, u_1, u_2, q_2,
          ...
          scalar_components, V, Q, GradFunction, DivFunction, dp_, **NS_namespace):
    ...
    # Pressure update
    Fp = (inner(grad(q), grad(p) - grad(p_) + nu * grad(div(u_))) * dx + (1.0 / dt) * div(u_) * q * dx)
    ...
    return dict(F=F, Fu=Fu, Fp=Fp, divu=divu, gradp=gradp)
...
def pressure_solve(Fp, p_, bcs, dp_, x_, u_, q_, Q, nu, divu, **NS_namespace):
...
def velocity_update(u_components, q_, bcs, Fu, dp_, gradp, V, dt, x_, **NS_namespace):
