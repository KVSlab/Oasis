...
def mesh(N, **params):
    ...
    m = RectangleMesh(Point(-1.0, -1.0), Point(1.0, 1.0), N, N)
    return m


def create_bcs(ux_e, uy_e, p_e, V, Q, sys_comp, mesh, **NS_namespace):
    external_v = AutoSubDomain(lambda x, b: b )
    boundary_v = MeshFunction("size_t", mesh, mesh.topology().dim() - 1)
    boundary_v.set_all(0)
    external_v.mark(boundary_v, 1)
    #external_p = AutoSubDomain(lambda x, b: b and near(x[1],-1) and near(x[0],-1) )
    #boundary_p = MeshFunction("size_t", mesh, mesh.topology().dim() - 1)
    #boundary_p.set_all(0)
    #external_p.mark(boundary_p, 1)
    #f = File("test.pvd")
    #f << boundary_p

    bcs = dict((ui, []) for ui in sys_comp)
    bc0 = DirichletBC(V, ux_e, boundary_v, 1)
    bc1 = DirichletBC(V, uy_e, boundary_v, 1)
    #bcp = DirichletBC(Q, p_e, boundary_p, 1)
    bcp = DirichletBC(Q, p_e, 'near(x[1],-1) && near(x[0],-1)')
    bcs['u0'] = [bc0]
    bcs['u1'] = [bc1]
    bcs['p']  = [bcp]
...
def theend_hook(V, Q, ux_e, uy_e, q_, T, dt, mesh, p_e, ux_error, uy_error, p_error, **NS_namespace):
    ux = interpolate(ux_e, V)
    uy = interpolate(uy_e, V)
    p  = interpolate(p_e, Q)
    print(" ")
    ...
    # print("L2 11 norm (ux-uxmms) {0:.6e}".format(E))
    # print("L2 22 norm (ux-uxmms) {0:.6e}".format(norm(ux_error,'l2')))
    # print("L2 33 norm (ux-uxmms) {0:.6e}".format(norm(ux_error.vector(),'l2')))
    # print("L2 44 norm (ux-uxmms) {0:.6e}".format(ux_error.vector().norm('l2')))
    # print("Linf 44 norm (ux-uxmms) {0:.6e}".format(ux_error.vector().norm('linf')))
    #
    # print("L2 norm (ux-uxmms) {0:.6e}".format(norm(ux_error.vector(),'l2')))
    # print("L2 norm (uy-uymms) {0:.6e}".format(norm(uy_error.vector(),'l2')))
    # print("L2 norm (p-pmms)   {0:.6e}".format(norm(p_error.vector(),'l2')))
    # print(" ")
    # print("Linf norm (ux-uxmms) {0:.6e}".format(norm(ux_error.vector(),'linf')))
    # print("Linf norm (uy-uymms) {0:.6e}".format(norm(uy_error.vector(),'linf')))
    # print("Linf norm (p-pmms)   {0:.6e}".format(norm(p_error.vector(),'linf')))
